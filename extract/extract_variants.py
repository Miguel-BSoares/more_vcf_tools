...
def get_genotype_string(gt):
    """
    gt: tuple/list of three elements: (allele1, allele2, phased)
    Returns a genotype string, e.g., "0/1", "1|0", or "./."
    """
...
def main():
    ...
    # We'll output: CHROM, POS, ID (if available), REF, ALT, AlleleFreqRef, AlleleFreqAlt, GenotypeStrings (sample-wise)
    samples = vcf.samples
    with open(allele_dist_path, 'w') as ad_out, open(no_snp_path, 'w') as no_snp_out:
        header_cols = ["CHROM", "POS", "ID", "REF", "ALT", "AlleleFreqRef", "AlleleFreqAlt"] + [f"{s}_GT" for s in samples]
        ...
        for chrom, start, end, coord_str in regions:
            ...
            for variant in vcf(f"{chrom}:{start}-{end}"):
                ...
                # Genotype strings per sample
                gt_strings = []
                for gt in variant.genotypes:
                    gt_str = get_genotype_string(gt)
                ...
                out_fields = [chrom_v, str(pos_v), id_v, ref_v, alt_v,
                              f"{allele_freq_ref:.5f}", f"{allele_freq_alt:.5f}"] + gt_strings
